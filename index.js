...
User @${ngrm.split("@")[0]} Mengirim Reaction ${
...
mans.ev.on('group-participants.update', async (anu) => {
        ...
        try {
         ...
         mans.sendMessage(anu.id, { image: { url: cug }, contextInfo: { mentionedJid: [num] }, caption: `Hai @${num.split("@")[0]} Selamat Datang Di ${metadata.subject}` })
                    ...
                    mans.sendMessage(anu.id, { image: { url: cugy }, contextInfo: { mentionedJid: [num] }, caption: `Soyonara @${num.split("@")[0]}` })
                }
            }
        } catch (err) {
            console.log(err)
